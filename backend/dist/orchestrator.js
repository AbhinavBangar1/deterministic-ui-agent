...
import { PLANNER_PROMPT, CODE_GENERATOR_PROMPT, EXPLAINER_PROMPT, MODIFICATION_PLANNER_PROMPT, buildPlannerMessage, buildModificationMessage, buildGeneratorMessage, buildExplainerMessage } from './prompts.js';
import { stateManager } from './stateManager.js';
import { callClaudeAPI } from './services/geminiClient.js';
async function callPlannerAgent(userRequest, isModification = false) {
...
async function callCodeGenerator(plan) {
...
async function callExplainer(userRequest, plan, isModification) {
    const userMessage = buildExplainerMessage(userRequest, JSON.stringify(plan, null, 2), isModification);
    const response = await callClaudeAPI(EXPLAINER_PROMPT, userMessage);
    return response.trim();
}
async function newUIFlow(userRequest) {
    try {
...
async function modifyUIFlow(userRequest) {
    try {
...
export async function orchestrate(userInput, flowType) {
