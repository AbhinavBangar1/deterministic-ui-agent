...
${COMPONENT_LIBRARY.components.map((comp) => `- ${comp.name}: ${comp.allowedProps.join(', ')}`).join('\n')}
...
export function buildPlannerMessage(userRequest) {
    return `User Request: ${userRequest}`;
}
export function buildModificationMessage(userRequest, currentPlan) {
    return `User Request: ${userRequest}\n\nCurrent Plan:\n${currentPlan}`;
}
export function buildGeneratorMessage(plan) {
    return `Plan:\n${plan}`;
}
export function buildExplainerMessage(userRequest, plan, isModification) {
